...
Then('New category should be added',() => {
    cy.get('@listLength').should('eq',13).then((resp) => {
...
Then('Category should be edited',() => {
    cy.get('[data-testid="category-card-name"]').last().should('have.text',editCategoryName)
})
...
Then('Category should be deleted',() => {
    cy.get('[data-testid="category-card-name"]').last().should('not.have.value',editCategoryName)
})