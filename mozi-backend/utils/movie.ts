...
export function getMovies(input: any, context: MyContext): Promise<Movie[]> {
  ...
  let params: any[] = [];
  if (input.searchField || input.category.length !== 0)
  ...
  if (input.searchField) {
    ...
    } else sql = sql.concat(` title LIKE CONCAT('%', ? , '%')`);
  }
  if (input.searchField && input.category.length !== 0)
    sql = sql.concat(` AND`);
  ...
  if (input.orderByCategory !== null && input.orderByTitle !== null)
    throw new Error("BOTH ORDERS");
  if (input.orderByCategory !== null) {
    if (input.orderByCategory === true)
      sql = sql.concat(` ORDER BY category_id`);
...
export async function createMovie(
  ...
  if (!validation)
    throw new GraphQLError(NOT_VALID_MOVIE, {
